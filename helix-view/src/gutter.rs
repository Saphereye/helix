...
pub fn line_numbers<'doc>(
    ...
    let draw_last = if doc.is_hex_dump() {
        true
    } else {
        let text = doc.text().slice(..);
        text.line_to_byte(last_line_in_view) < text.len_bytes()
    };
    ...
    let current_line =
        doc.display_char_to_line(doc.display_cursor(doc.selection(view.id).primary()));
...
fn line_numbers_width(view: &View, doc: &Document) -> usize {
    let last_drawn = if doc.is_hex_dump() {
        doc.display_len_lines()
    } else {
        let text = doc.text();
        let last_line = text.len_lines().saturating_sub(1);
        let draw_last = text.line_to_byte(last_line) < text.len_bytes();
        if draw_last { last_line + 1 } else { last_line }
    };
